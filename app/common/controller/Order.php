...
trait Order
{
    ...
    public function lst(Request $request)
    {
        ...
        if ($where['supplier_id'] || in_array($where['plat_type'], [0, 2])) {
            $where['pid'] = 0;
        } elseif (!$where['real_name'] && !in_array($where['status'], [-1, -2, -3])) {
            $where['pid'] = [0, -1];
        }
        $where['type'] = trim($where['type'], ' ');
    ...
    public function split_order(Request $request, $id)
    {
        ...
        return app('json')->success($this->services->getSplitOrderList($where, ['*'], ['split', 'pink', 'invoice', 'supplier', 'storeName']));
    }
    ...
    public function getUpdateInfo(StoreOrderCartInfoServices $cartInfoServices, $id)
    {
        ...
        $cartInfoList = $cartInfoServices->getColumn(['oid' => $id], 'cart_info');
        if (!$cartInfoList) {
    ...
    public function update($id)
    {
        ...
        if ($data['cart_info']) {
            foreach ($data['cart_info'] as $cart) {
                if (!isset($cart['id']) || !$cart['id'] || !isset($cart['true_price']) || !$cart['true_price']) {
    ...
    public function update_delivery(Request $request, StoreOrderDeliveryServices $services, $id)
    {
        ...
        return app('json')->success('SUCCESS', $services->delivery((int)$id, $data));
    }
        ...
        if ($data['cart_ids']) {
            ...
            foreach ($data['cart_ids'] as $cart) {
                ...
                if (isset($cartList[$cart['cart_id']])) {
                    ...
                    $weightnew = bcmul($value['productInfo']['attrInfo']['weight'], (string)$cart['cart_num'], 2);
            ...
            foreach ($orderCartInfoList as $item) {
                $weightnew = bcmul($item['productInfo']['attrInfo']['weight'], (string)$item['cart_num'], 2);
                $weight = bcadd($weightnew, $weight, 2);
            }
        }
    ...
    public function wirteoff(Request $request, WriteOffOrderServices $writeOffOrderServices, $order_id)
    {
        ...
        return app('json')->success('核销成功', $writeOffOrderServices->writeoffOrder(0, $orderInfo->toArray(), $cart_ids, 0, 0, (int)$request->adminId()));
    }
    ...
    public function update_refund(Request $request, StoreOrderRefundServices $services, StoreOrderCreateServices $storeOrderCreateServices, StoreOrderCartInfoServices $storeOrderCartInfoServices, $id)
    {
        ...
        if ($services->cacheHander()->has((string)$adminId)) {
            return app('json')->fail('请勿重复操作!');
        }
        $services->cacheTag()->set((string)$adminId, 1, 1);
        ...
        if ($type == 2) {
            ...
            $adminAccount = $this->adminInfo['account'] ?? '';
            //修改订单退款状态
            if ($services->agreeRefund($res->id, $refund_data, $adminAccount)) {
                //主动退款清楚原本退款单
...
//        $orderInfo['total_price'] = floatval(bcsub((string)$orderInfo['total_price'], (string)$vipTruePrice, 2));
    ...
    public function get_express(ExpressServices $services, $id)
    {
        ...
        if (!str_contains($orderInfo['delivery_id'], ':') && $orderInfo['user_phone'] && ($orderInfo['delivery_code'] == 'shunfengkuaiyun' || $orderInfo['delivery_code'] == 'zhongtong')) {
            $delivery_id = $orderInfo['delivery_id'] . ':' . substr($orderInfo['user_phone'], -4);
        }
        $data['result'] = $delivery_id ? $services->query($cacheName, $delivery_id) : [];
        return app('json')->success($data);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function writeoffFrom(Request $request, WriteOffOrderServices $writeOffOrderServices, $id)
    {
        if (!$id) {
            return $this->fail('缺少核销订单ID');
        }
        ...
        return app('json')->success('核销成功', $writeOffOrderServices->writeoffOrder(0, $orderInfo->toArray(), $cart_ids, 0, 0, $adminId));
    }
