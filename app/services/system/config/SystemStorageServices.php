...
class SystemStorageServices extends BaseServices
{
    ...
    /**
     * @var SystemStorageDao
     */
    #[Inject]
    protected SystemStorageDao $dao;
    ...
    public function getFormStorageConfig(int $type)
    {
        ...
        if ($type === 5) {
            $rule[] = FormBuilder::input('storageRegion', 'Region', $config['storageRegion'] ?? '')->required();
        }
        ...
        if (1 !== $type) {
            ...
            switch ($type) {
                ...
                case 5:// oss 京东云
                    ...
                    $make->update('jd_storage_region', ['value' => json_encode($storageRegion)], 'menu_name');
                    break;
                case 6:// oss 华为云
    ...
    public function synchronization(int $type)
    {
        ...
        switch ($type) {
            ...
            case 5:// cos 京东云
                ...
                $list = $upload->listbuckets();
                $config = $this->getStorageConfig($type);
                $location = $config['storageRegion'] ?? 'cn-north-1';
                foreach ($list as $item) {
    ...
    {
        $res = ['name' => '', 'region' => '', 'domain' => '', 'cname' => ''];
        try {
            $config = $this->dao->get(['type' => $type, 'status' => 1, 'is_delete' => 0]);
            if ($config) {
                return ['name' => $config->name, 'region' => $config->region, 'domain' => $config->domain, 'cname' => $config->cname];
            }
        } catch (\Throwable $e) {
