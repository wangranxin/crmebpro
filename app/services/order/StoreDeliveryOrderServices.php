...
class StoreDeliveryOrderServices extends BaseServices
{
     ...
     * @var string
     */
    public string $platCityShopId = 'plat_delivery_city_shop_001';

    /**
     * @var StoreDeliveryOrderDao
     */
    #[Inject]
    protected StoreDeliveryOrderDao $dao;
...
//        $locationOption->setRegion($region);
     ...
     * @throws \think\db\exception\ModelNotFoundException
     * User: liusl
     * DateTime: 2024/12/25 下午4:54
     */
    public function orderDetail(int $oid)
    {
        // 获取订单数据
        $res = $this->dao->get(['oid' => $oid]);
        if (!$res) {
            return [
                'status_name' => '',
                'porter_name' => '',
    ...
    public function doCancel(int $id, $deliveryOrder, $deduct_fee, $mark)
    {
        ...
        //记录订单状态
        OrderStatusJob::dispatch([$deliveryOrder['oid'], 'city_delivery_cancel', ['change_message' => '同城配送取消']]);
        return true;
    }
    ...
    public function notifyAfter($status, $reason, $deliveryOrder, $data, $deductFee)
    {
        ...
        if ($orderData['status'] != $status && !$statusServices->count(['oid' => $oid, 'change_type' => $change_type])) {
            ...
            //记录原订单状态
            OrderStatusJob::dispatch([$oid, $change_type, ['change_message' => $message]]);
            switch ($status) {
                case 2:
    ...
    public function syncCityShop(int $id, int $type = 1, int $station_type = 1)
    {
        ...
        if (!isset($station['latitude'])) {//地址转经纬度
            ...
            if (!$addressArr['province'] || !$addressArr['city']) {
                throw new ValidateException($type == 0 ? '退货收货地址格错误，应包含完整的省市地址' : '请检查该订单关联供应商地址信息是否正确');
            }
