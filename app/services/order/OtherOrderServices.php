...
class OtherOrderServices extends BaseServices
{
    ...
    public function isCanGetFree(int $uid)
    {
        ...
        $user = $userService->getUserCacheInfo($uid);
        $userInfo = [
            'uid' => $user['uid'],
            ...
            'is_ever_level' => $user['is_ever_level'],
            'is_money_level' => $user['is_money_level'],
            'overdue_time' => $user['overdue_time'],
        ];
        /** @var TradeStatisticServices $tradeService */
        $tradeService = app()->make(TradeStatisticServices::class);
        $registerTime = $tradeService->TimeConvert(['start_time' => date('Y-m-d H:i:s', $user['add_time']), 'end_time' => date('Y-m-d H:i:s', time())]);
     ...
     * @throws \Exception
     */
    public function createOrder(int $uid, string $channelType, string $memberType, string $payPrice, string $payType, $type, $money = 0, int $store_id = 0, int $staff_id = 0)
    {
        ...
        if ($type != 3) { //区别 0：免费领取会员 1：购买会员  2：卡密领取会员  3：线下付款
            ...
            [$memberPrice, $isFree, $isPermanent, $overdueTime, $newMemberRight] = $this->checkPayMemberType($uid, (int)$memberType);
            $orderInfo['member_price'] = $memberPrice;
            $orderInfo['money'] = $memberPrice;
            $orderInfo['vip_day'] = $newMemberRight['vip_day'];
...
//        }
    ...
    public function getScanOrderList(array $where)
    {
        ...
        if ($list) {
            ...
            foreach ($list as &$v) {
                ...
                $v['pay_type'] = PayServices::PAY_TYPE[$v['pay_type']] ?? '其他方式';
    ...
    public function time(int $store_id, int $staff_id, array $time = [])
    {
        ...
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->getMemberRecord($where + ['time' => [$start, $stop]], 'id,order_id,uid,pay_price,add_time', ['userInfo'], $page, $limit);
