...
class StoreOrderSplitServices extends BaseServices
{
     ...
     * @var string[]
     */
    protected array $order_data = ['id', 'refund_status', 'refund_type', 'refund_express', 'refund_reason_wap_img', 'refund_reason_wap_explain', 'refund_reason_time', 'refund_reason_wap', 'refund_reason', 'refund_price'];
...
//                $order_data['order_id'] = $storeOrderCreateServices->getNewOrderId();
            ...
            OrderStatusJob::dispatch([$new_id, $change_type, ['change_message' => $change_message, 'change_manager_type' => 'system']]);
...
//                $cart_data['surplus_num'] = $cart['cart_num'];
            ...
            if (!$i) {
				//无核销 ｜｜ 核销数量大于拆分数量
				$surplus_num = min($cart_data['write_surplus_times'], $cart_num_times);
                if ($is_refund) $refund_num = $cart['cart_num'];
            } else {//修改原订单
                //无核销 ｜｜ 核销数量大于拆分数量
                if (!$cart_data['writeoff_time']) {
                    $surplus_num = $cart_num_times;
                } else {
					$writoff_numm = bcsub((string)$cart_data['write_times'], (string)$cart_data['write_surplus_times'], 0);
					$surplus_num = $writoff_numm >= $cart_num_times ? 0 : bcsub((string)$cart_num_times, (string)$writoff_numm, 0);
                }
            ...
            $cart_data['refund_num'] = max($refund_num, 0);
            $cart_data['write_surplus_times'] = max($surplus_num, 0);
            if (!$cart_data['write_surplus_times']) {//核销完毕
                $cart_data['is_writeoff'] = 1;
            } elseif ($cart_data['write_surplus_times'] == $cart_data['write_times']) {//未核销
...
//        return $split_order_info;
    ...
    public function splitComputeOrder(int $id, array $cart_info_data, $orderInfo = [], float $order_pay_price = 0.00, float $pay_price = 0.00, float $pre_pay_price = 0.00)
    {
        ...
        $cost_price = $total_price = $true_total_price = $true_total_integral = $coupon_price = $deduction_price = $use_integral = $pay_postage = $gainIntegral = $one_brokerage = $two_brokerage = $promotions_price = $first_order_price = 0;
        ...
        foreach ($cart_info_data as $cart) {
			...
			$true_total_integral = bcadd((string)$true_total_integral, (string)bcmul((string)($cart['cart_num'] ?? '1'), (string)($_info['integral'] ?? '0'), 0), 0);
            $total_price = bcadd((string)$total_price, (string)bcmul((string)($cart['cart_num'] ?? '1'), (string)($_info['sum_price'] ?? '0'), 2), 2);
			$cost_price = bcadd((string)$cost_price, (string)bcmul((string)($cart['cart_num'] ?? '1'), (string)($_info['costPrice'] ?? '0'), 2), 2);
        ...
        $order_update['pay_price'] = max($order_update['pay_price'], 0);
		$order_update['pay_integral'] = $true_total_integral;
        $order_update['total_price'] = $total_price;
		$order_update['cost'] = $cost_price;
    ...
    public function slpitComputeOrderCart(int $cart_num, array $cart_info, int $i = 0, bool $is_gift = false)
    {
        ...
        if ($cart_num < $cart_info['cart_num']) {
            $compute_arr = ['coupon_price', 'integral_price', 'postage_price', 'use_integral', 'one_brokerage', 'two_brokerage', 'sum_true_price', 'first_order_price', 'division_staff_brokerage', 'division_agent_brokerage', 'division_brokerage'];
