...
class SupplierOrderServices extends BaseServices
{
    ...
    {
        $orderList = $this->dao->getList(['id' => $id], ['id', 'order_id', 'trade_no', 'real_name', 'user_phone', 'user_address', 'pay_time', 'pay_type', 'pay_postage as freight_price', 'coupon_price', 'deduction_price', 'use_integral', 'pay_price', 'mark','first_order_price']);
        if (!$orderList) {
    ...
    {
        $bing_xdata = [0 => '普通订单', 1 => '秒杀订单', 2 => '砍价订单', 3 => '拼团订单', 4 => '积分订单', 5 => '套餐订单', 6 => '预售订单', 7 => '新人专享', 8 => '抽奖订单'];
        $color = ['#64a1f4', '#3edeb5', '#70869f', '#ffc653', '#ffc653', '#fc7d6a', '#fc7d2a', '#ffc653', '#ffc653'];
        $bing_data = [];
    ...
    public function deliverRemind(int $supplierId, int $id)
    {
        ...
        //向供应商后台发送待发货订单消息
        SocketPushJob::dispatch([$supplierId, 'WAIT_DELIVER_ORDER', ['order_id' => $order['order_id']], 'supplier']);
