...
class StorePromotionsServices extends BaseServices
{
     ...
     * @var array
     */
    protected array $promotionsData = [
        ...
        if ($ids) {
            ...
            if ($promotions) {
                ...
                foreach ($promotions as &$item) {
                    ...
                    if (isset($item['giveProducts']) && $item['giveProducts']) {
                        $item['giveProducts'] = array_column($item['giveProducts'], 'productInfo');
                    }

                    if (isset($item['giveCoupon']) && $item['giveCoupon']) {
                        $item['giveCoupon'] = array_column($item['giveCoupon'], 'coupon');
                    }
     ...
     * @return array
     */
    public function getUsePromotiosnInfo(int $uid, array $usePromotionsIds, array $promotionsArr, array $computedArr)
    {
        $usePromotions = [];
        ...
        foreach ($usePromotionsIds as $id) {
            ...
            if (isset($promotionsInfo['give_product']) && $promotionsInfo['give_product']) {
                $giveCart = $this->createGiveProductCart($uid, (int)$id, $promotionsInfo['give_product'], $promotionsInfo);
                if ($giveCart) {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function computedPromotions(int $uid, array $cartList, int $couponId = 0, bool $isCart = false)
    {
        $giveIntegral = $couponPrice = 0;
        $giveCoupon = $giveCartList = $usePromotions = $useCoupon = [];
        if ($cartList) {
            ...
            if ($promotionsArr) {
                ...
                foreach ($cartList as &$cart) {
                    $sum_promotions_true_price = 0;//所有单件优惠金额累加
                    $sum_promotions_price = 0;//所有优惠金额累加
                    ...
                    $total_price = $cart['total_price'] ?? bcmul((string)$price, (string)$cart['cart_num'], 2);
                    ...
                    foreach ($promotionsIds as $promotions_id) {
                        ...
                        if (!isset($trueDetail['is_valid']) || $trueDetail['is_valid'] == 0) {
                            ...
                            if ($isOverly) {
                                ...
                                $sum_promotions_price = bcadd((string)$sum_promotions_price, (string)($trueArr['sum_promotions_price'] ?? 0), 2);
                            } else { //不叠加取最优惠
                                if ($isCart) {
                                    $cart['promotions_id'][] = $promotions_id;
                                }
                                if ($sum_promotions_true_price < ($trueArr['promotions_true_price'] ?? 0)) {
                                    ...
                                    $sum_promotions_price = $trueArr['sum_promotions_price'] ?? 0;
                                    if (!$isCart) {
                                        $cart['promotions_id'] = [$promotions_id];
                                    }
                                }
                    ...
                    if ($sum_promotions_true_price) {
                        ...
                        if (in_array(1, $typeArr)) {
                            ...
                            $pay_price = (float)bcsub((string)$total_price, (string)$sum_promotions_price, 2);
                            $true_price = max($true_price, 0);
                            $pay_price = max($pay_price, 0);

                            //比较与用户等级、svip优惠后金额
                            if ($true_price && $cart['truePrice'] > $true_price) {
                                $cart['truePrice'] = $true_price;
                                $cart['pay_price'] = $pay_price;
                                $cart['promotions_true_price'] = $sum_promotions_true_price;
                                $cart['sum_promotions_price'] = $sum_promotions_price;
                            ...
                            $pay_price = (float)bcsub((string)$cart['pay_price'], (string)$sum_promotions_price, 2);
                            $cart['truePrice'] = max($true_price, 0);
                            $cart['pay_price'] = max($pay_price, 0);
                            $cart['promotions_true_price'] = $sum_promotions_true_price;
                            $cart['sum_promotions_price'] = $sum_promotions_price;
                            $cart['price_type'] = 'promotions';
                        }
                    ...
                    foreach ($promotionsIds as $promotions_id) {
                        ...
                        $sum_promotions_price = bcsub((string)($trueDetail['sum_promotions_price'] ?? '0'), (string)($trueArr['sum_promotions_price'] ?? 0), 2);
                        $computedArr[$promotions_id]['sum_promotions_price'] = max($sum_promotions_price, 0);//这个商品的没有使用优惠活动
            ...
            $coupon_id = 0;
            if ($uid) {
                if ($usePromotionsIds) [$usePromotions, $giveIntegral, $giveCoupon, $giveCartList] = $this->getUsePromotiosnInfo($uid, $usePromotionsIds, $promotionsArr, $computedArr);
                if ($couponId) {
                    [$useCoupon, $couponPrice] = $this->useCoupon($couponId, $uid, $cartList, $usePromotions);
                    $coupon_id = $couponId;
                } else {
                    //获取最优优惠券
                    if ($isCart) {
                        ...
                        $useCoupon = $couponServices->getCanUseCoupon($uid, $cartList, $usePromotions);
                        $couponServices->reset();
                        $couponPrice = $useCoupon['true_coupon_price'] ?? 0;
                        $coupon_id = $useCoupon['used']['id'] ?? 0;
                    }
                }
                //计算每一件商品优惠券优惠金额
                if ($coupon_id && $useCoupon && $couponPrice) {
                    /** @var StoreOrderComputedServices $computedServices */
                    $computedServices = app()->make(StoreOrderComputedServices::class);
                    $payPrice = $computedServices->getOrderSumPrice($cartList, 'pay_price', 'false');
        ...
        return [$cartList, $couponPrice, $useCoupon, $usePromotions, $giveIntegral, $giveCoupon, $giveCartList];
    }
                    ...
                    if ($p) {
                        ...
                        if ($promotions['is_limit']) {//是否限量
                            ...
                            foreach ($promotionsProductArr as $product) {
                                ...
                                $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)$cartNum, 2);
                                if ($promotions_true_price < 0.01) {//实际优惠小于0.01 就不计算优惠
                                    $promotions_true_price = 0;
                                }
                                //部分享受折扣
                                if ($cartNum != $newCartNum) {
                                    $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)$newCartNum, 2);
                                    $promotions_true_price = bcdiv((string)$one_promotions_sum_price, (string)$cartNum, 2);
                                }
                                $data[$unique] = ['product_id' => $product_id, 'unique' => $unique, 'price' => $price, 'sum_promotions_price' => $one_promotions_sum_price, 'promotions_true_price' => $promotions_true_price];
                            ...
                            foreach ($promotionsProductArr as $product) {
                                ...
                                $price = $product['price'];
                                $promotions_true_price = (float)bcmul((string)$price, (string)$promotionsDiscount, 2);
                                $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)($product['cart_num'] ?? 1), 2);
                                if ($promotions_true_price < 0.01) {//实际优惠小于0.01 就不计算优惠
                                    $promotions_true_price = 0;
                                }
                                $data[$unique] = ['product_id' => $product_id, 'unique' => $unique, 'price' => $price, 'sum_promotions_price' => $one_promotions_sum_price, 'promotions_true_price' => $promotions_true_price];
                                $sum_promotions_price = bcadd((string)$sum_promotions_price, (string)$one_promotions_sum_price, 2);
                            }
                        }
                        $data['is_valid'] = 1;
                        ...
                        if ($sumCount >= $p['threshold']) {//满足
                            ...
                            if ($sum_promotions_price) {
                                ...
                                foreach ($useProductArr as $value) {
                                    ...
                                    $price = $value['price'];
                                    if ($count > 1) {
                                        $promotions_true_price = (float)bcmul((string)bcdiv((string)$price, (string)$useSumPrice, 4), (string)$sum_promotions_price, 2);
                                        $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)$value['cart_num'], 2);
                                        $compute_price = (float)bcadd((string)$compute_price, (string)$one_promotions_sum_price, 2);
                                    } else {
                                        $one_promotions_sum_price = (float)bcsub((string)$sum_promotions_price, (string)$compute_price, 2);
                                        $promotions_true_price = (float)bcdiv((string)$one_promotions_sum_price, (string)$value['cart_num'], 2);
                                    }
                                    $data[$unique] = ['product_id' => $product_id, 'unique' => $unique, 'price' => $price, 'sum_promotions_price' => $one_promotions_sum_price, 'promotions_true_price' => $promotions_true_price];
                                    $count--;
                                }
                        ...
                        if ($valid) {
                            ...
                            foreach ($promotionsProductArr as $product) {
                                ...
                                if ($valid['discount_type'] == 1) {
                                    if ($productCount > 1) {
                                        $promotions_true_price = (float)bcmul((string)bcdiv((string)$price, (string)$sumPrice, 4), (string)$sum_promotions_price, 2);
                                        $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)$product['cart_num'], 2);
                                        $compute_price = bcadd((string)$compute_price, (string)$one_promotions_sum_price, 2);
                                    } else {
                                        $one_promotions_sum_price = (float)bcsub((string)$sum_promotions_price, (string)$compute_price, 2);
                                        $promotions_true_price = (float)bcdiv((string)$one_promotions_sum_price, (string)$product['cart_num'], 2);
                                    }
                                } else {
                                    $promotions_true_price = (float)bcmul((string)$price, (string)$promotionsDiscount, 2);
                                    if ($promotions_true_price < 0.01) {//实际优惠小于0.01 就不计算优惠
                                    ...
                                    $one_promotions_sum_price = (float)bcmul((string)$promotions_true_price, (string)($product['cart_num'] ?? 1), 2);
                                    $sum_promotions_price = (float)bcadd((string)$sum_promotions_price, (string)$one_promotions_sum_price, 2);
                                }
                                $data[$unique] = ['product_id' => $product_id, 'unique' => $unique, 'price' => $price, 'sum_promotions_price' => $one_promotions_sum_price, 'promotions_true_price' => $promotions_true_price];
                                $productCount--;
                            }
                ...
                if ($validCount) {//满足次数
                    ...
                    if ($p['giveProducts']) {
                        foreach ($p['giveProducts'] as $value) {
                            if (!$value || !isset($value['surplus_num']) || !$value['surplus_num']) continue;
                            $minValidCunt = min($validCount, $value['surplus_num']);
                            if (isset($give_product_id[$value['unique']])) {
                                $give_product_id[$value['unique']]['cart_num'] = bcadd((string)$give_product_id[$value['unique']]['cart_num'], (string)$minValidCunt);
                            } else {
                                $give_product_id[$value['unique']] = ['promotions_id' => $value['promotions_id'], 'unique' => $value['unique'], 'product_id' => $value['product_id'], 'cart_num' => $minValidCunt];
                            }
                        }
                    }
                    $data['is_valid'] = 1;
    ...
    public function createGiveProductCart(int $uid, int $promotions_id, array $giveProduct, array $promotions, int $store_id = 0)
    {
        ...
        if ($giveProduct) {
            ...
            foreach ($giveProduct as $unique => $give) {
                ...
                try {
                    [$attrInfo, $product_attr_unique, $bargainPriceMin, $cart_num, $productInfo] = $storeCartServices->checkProductStock($uid, (int)$product_id, (int)$cart_num, $unique, true, 0, 0, 0, true);
                } catch (\Throwable $e) {
                    continue;
                }
                $is_limit = false;
                ...
                if (isset($info['productInfo']['delivery_type'])) {
                    ...
                    $info['productInfo']['delivery_type'] = [];
                }
                ...
                $info['total_price'] = bcmul((string)$info['sum_price'], (string)$info['cart_num'], 2);
                $info['pay_price'] = $info['truePrice'] = 0;
