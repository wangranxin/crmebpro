...
class UserLevelServices extends BaseServices
{
        ...
        if ($uid && sys_config('member_func_status', 0)) {
            ...
            if ($userInfo) {
                ...
                if (!$levelInfo) {//不存在等级 展示最低等级
                    ...
                    $alllevelInfo = $systemUserLevel->getList([['is_del', '=', 0], ['is_show', '=', 1]], 'id,name,icon,grade,discount', 1, 1);
    ...
    public function setUserLevel(int $uid, int $level_id, $vipinfo = [])
    {
        ...
        if (!$user->update(['uid' => $uid], ['level' => $level_id, 'exp' => $vipinfo['exp_num'], 'level_status' => 1]))
...
//        $field[] = Form::number('valid_date', '有效时间(天)', $vipinfo['valid_date'] ?? 0)->min(0)->col(12);
        ...
        $field[] = Form::number('discount', '享受折扣', $vipinfo['discount'] ?? 100)->min(0)->max(100)->info('输入折扣数100，代表划线价，90代表9折')->placeholder('输入折扣数100，代表原价，90代表9折');
        ...
        $field[] = Form::frameImage('image', '会员背景(750*990)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'image')), $vipinfo['image'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true])->appendValidate(Iview::validateStr()->required()->message('请选择背景'));
        ...
        $field[] = Form::textarea('explain', '等级说明', $vipinfo['explain'] ?? '')->required('请添加等级说明');
