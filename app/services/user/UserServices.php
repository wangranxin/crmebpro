...
class UserServices extends BaseServices
{
    ...
    public function checkIsVip(array $uid): array
    {
        ...
        foreach ($userList as $item) {
            $level_name = '';
            $vip_status = $is_open_member && $item && isset($item['is_money_level']) && $item['is_money_level'] > 0 && isset($item['overdue_time']) && $item['overdue_time'] > time() ? 1 : 0;
            if ($is_open_level && $item['level']) {
        ...
        if ($list) {
            ...
            foreach ($list as &$item) {
                ...
                if ($item['is_ever_level'] == 1) {
                    ...
                    if ($item['is_money_level'] > 0 && $item['overdue_time'] > 0) {
        ...
        if (!isset($data['is_other']) || !$data['is_other']) {
            ...
            if ($data['spread_uid'] != -1) {
                ...
                if ($data['spread_uid'] > 0) {
                    ...
                    switch ($user['division_type']) {
                        case 1:
                            $edit['division_id'] = $user['uid'];
                            $edit['agent_id'] = 0;
                            $edit['staff_id'] = 0;
                            break;
                        case 2:
                            $edit['division_id'] = $spreadInfo['division_id'];
                            $edit['agent_id'] = $user['uid'];
                            $edit['staff_id'] = 0;
                            break;
                        case 3:
                            $edit['division_id'] = $spreadInfo['division_id'];
                            $edit['agent_id'] = $spreadInfo['agent_id'];
                            $edit['staff_id'] = $user['uid'];
                            break;
                    }
...
//
    ...
    public function saveUserSpreadUid(int $uid, int $spread_uid)
    {
        ...
        switch ($userInfo['division_type']) {
            case 1:
                $data['division_id'] = $userInfo['uid'];
                $data['agent_id'] = 0;
                $data['staff_id'] = 0;
                break;
            case 2:
                $data['division_id'] = $spreadInfo['division_id'];
                $data['agent_id'] = $userInfo['uid'];
                $data['staff_id'] = 0;
                break;
            case 3:
                $data['division_id'] = $spreadInfo['division_id'];
