...
class StoreOrderDao extends BaseDao
{
     ...
     * @var string[]
     */
    protected array $withField = ['uid', 'order_id', 'real_name', 'user_phone', 'title', 'total_num'];
    ...
    public function search(array $where = [], bool $search = false)
    {
        ...
        return parent::search($where, $search)->when($isDel, function ($query) use ($where) {
            if(is_array($where['is_del'])){
                $query->whereIn('is_del', $where['is_del']);
            }else{
                ...
                case 2://已支付  待收货 待核销
                    $query->where('paid', 1)->where(function ($query) {
                        $query->where(function ($q) {
                            $q->whereIn('status', [1, 5])->where('shipping_type', 1);
                        })
                            ->whereOr(function ($e) {
                            $e->whereIn('status', [0, 5])->where('shipping_type', 2);
                        });
                    })->whereIn('refund_status', [0, 3])->where('is_del', 0);
                    break;
                case 3:// 已支付  已收货  待评价
        ...
        })->when(isset($where['division_id']) && $where['division_id'] != 0, function ($query) use ($where) {
            if ($where['division_id'] == -1) {
                $query->where('division_id', '>', 0);
            } else {
                $query->where('division_id', $where['division_id']);
            }
        })->when(isset($where['division_agent_id']) && $where['division_agent_id'] != 0, function ($query) use ($where) {
            if ($where['division_agent_id'] == -1) {
                $query->where('division_agent_id', '>', 0);
            } else {
                $query->where('division_agent_id', $where['division_agent_id']);
            }
        });
    }
     ...
     * @throws ModelNotFoundException
     */
    public function getOrderList(array $where, array $field, int $page = 0, int $limit = 0, array $with = [], string $order = 'add_time DESC,pay_time DESC,id DESC')
    {
        return $this->search($where, false)
            ->field($field)
            ->with(array_merge(['user', 'spread', 'refund'], $with))
            ->when($page && $limit, function ($query) use ($page, $limit, $where) {
                //兼容大数据分页查询
                if (!empty($where['pay_time']) && !empty($where['id'])) {
                    $query->where(function ($query) use ($where) {
                        $query->where('pay_time', '<', $where['pay_time'])
                    ...
                    $query->where('id', '<', $where['id'])->page($page, $limit);
                } else {
                    $query->page($page, $limit);
                }
     ...
     * 获取活动订单列表：
     * @param int $id
     * @param int $type 0:普通、1：秒杀、2:砍价、3:拼团、4:积分、5:套餐、6:预售、7:新人礼、8：抽奖
     * @param array $where
     * @param int $page
     * @param int $limit
     * @return array
     ...
     * @date 2024/8/21
     */
    public function getDivisionTrend($time, $timeType, $divisionId, $field)
    {
        return $this->getModel()
            ->where('paid', 1)
            ->where('pid', '>=', 0)
            ...
            })->field("FROM_UNIXTIME(add_time,'$timeType') as days,$field as num")->group('days')->select()->toArray();
    }
}
