...
class PublicController
{
    ...
    public function colorChange(DiyServices $services, $name)
    {
        ...
        return app('json')->success(compact('status', 'navigation', 'product_category_level'));
    }
	...
	* @throws \think\db\exception\ModelNotFoundException
    ...
    public function productDetailDiy(DiyServices $services)
    {
		...
		$store_brokerage_statu = sys_config('store_brokerage_statu');//分销模式 1,指定分销,2人人分销,3满额分销
		$store_brokerage_price = sys_config('store_brokerage_price');//满额分下金额
		$product_category = $services->getProductCategoryDiy();

        return app('json')->success(compact('store_brokerage_price','store_brokerage_statu','product_detail', 'product_video_status', 'product_category'));
    }
