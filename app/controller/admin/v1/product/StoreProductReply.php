...
class StoreProductReply extends AuthController
{
    ...
    public function getReply(StoreProductReplyCommentServices $services, $id)
    {
        ...
        $where = ['reply_id' => $id, 'uid' => 0, 'relation_id' => 0];
    ...
    {
        [$product_id] = $this->request->postMore([
            ['product_id', 0],
        ], true);
    ...
    public function save_fictitious_reply()
    {
        ...
        $this->validate(['product_id' => $data['product_id'], 'nickname' => $data['nickname'], 'avatar' => $data['avatar'], 'comment' => $data['comment'], 'reply_score' => $data['reply_score'], 'product_score' => $data['product_score'], 'service_score' => $data['service_score'], 'delivery_score' => $data['delivery_score']], \app\validate\admin\product\StoreProductReplyValidate::class, 'save');
