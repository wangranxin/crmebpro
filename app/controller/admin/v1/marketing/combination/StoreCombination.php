...
class StoreCombination extends AuthController
{
    ...
    public function set_status($id, $status)
    {
        ...
        // 下架拼团活动，将当前拼团商品所有的拼团订单的到期时间改为当前时间，自动成团或者退款
        if ($status == 0) {
            app()->make(StorePinkServices::class)->update(['cid' => $id, 'status' => 1], ['stop_time' => time()]);
        }
    ...
    {
        $data = $this->services->combinationStatistics((int)$id);
    ...
    public function combinationStatisticsOrder($id)
    {
        ...
        return app('json')->success($this->services->combinationStatisticsOrder((int)$id, $where));
    }
}
