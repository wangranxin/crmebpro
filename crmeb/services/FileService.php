...
class FileService
{
     ...
     * @throws \PhpOffice\PhpSpreadsheet\Reader\Exception
     * User: liusl
     * DateTime: 2024/12/9 下午4:07
     */
    public function readImportExcel($filePath, int $row_num = 1, $type = 'user', string $suffix = 'Xlsx')
    {
        if (!$filePath) return false;
        $pathInfo = pathinfo($filePath, PATHINFO_EXTENSION);
        if (!$pathInfo || $pathInfo != "xlsx") throw new ValidateException('必须上传xlsx格式文件');
        ...
        try {
            ...
            for ($i = $row_num; $i <= $highestRow; $i++) {
                ...
                for ($j = 1; $j <= $highestColumnIndex; $j++) {
                    // 获取单元格的值并转换为字符串
                    $cellValue = $this->objToStr($sheet->getCellByColumnAndRow($j, $i)->getValue()) ?? '';
                    // 将单元格的值添加到当前行数据数组中
                    $rowData[] = $cellValue;
                }
                //第一行标题,验证是否传入正确
                if ($i == $row_num) {
                    $this->getVerifyColumns($type, $rowData);
                } else {
                    // 如果是数据行，处理数据
                    $rowData = $this->processRowData($rowData, $type);
                    $data[] = $rowData;
                }
            }
            // 返回表格数据
     ...
     * @param $type
     * User: liusl
     * DateTime: 2024/12/21 上午9:37
     */
    protected function getVerifyColumns($type, $firstRowData)
    {
        $verifyColumns = match ($type) {
            'user' => [
            ...
            'product' => [
                '商品编号', '商品名称', '商品类型', '商品分类(一级)', '商品分类(二级)', '商品单位',
                '商品图片', '商品视频', '商品详情', '已售数量', '起购数量', '规格类型', '规格类型值',
     ...
     * DateTime: 2024/12/21 上午9:37
     */
    protected function processRowData(array $rowData, $type)
    {
        return match ($type) {
            'user' => array_combine(
