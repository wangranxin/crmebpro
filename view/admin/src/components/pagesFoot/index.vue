  ...
  >
    ...
    >
      ...
      <div v-if="navStyleConfig != 2">
        <p
          v-if="index == isSpecial"
          :style="{ color: toneConfig ? activeTxtColor : colorStyle.theme }"
        >
          {{ item.name }}
        </p>
        <p v-else :style="{ color: toneConfig ? txtColor : '#1A1A1A' }">
          {{ item.name }}
        </p>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapState("admin/mobildConfig", ["pageFooter"]),
  },
  watch: {
    pageFooter: {
      handler(nVal, oVal) {
  ...
  mounted() {
    let data = this.$store.state.admin.mobildConfig.pageFooter;
    this.setConfig(data);
  },
  methods: {
    setConfig(data) {
      ...
      this.bgRadius = fillet
        ? valList[0].val +
          ...
          "px"
        : filletVal + "px";
      this.$store.commit("admin/mobildConfig/footType", this.navConfig);
      this.$store.commit("admin/mobildConfig/footBottom", this.mTop);
      this.menuList = [];
