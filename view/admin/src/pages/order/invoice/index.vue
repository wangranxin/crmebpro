        ...
        <Card :bordered="false" dis-hover class="ivu-mt">
            ...
            <Table :columns="columns" :data="orderList" ref="table" class="mt25"
                ...
                <template slot-scope="{ row, index }" slot="is_invoice">
                    <Tag color="orange" size="medium" v-if="row.is_invoice===0">未开票</Tag>
                    <Tag color="green" size="medium" v-else-if="row.is_invoice===1">已开票</Tag>
                    <Tag color="red" size="medium" v-else>已拒绝</Tag>
                </template>
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <a :disabled="row.refund_status !== 0 || row.is_invoice == -1" @click="edit(row)">编辑</a>
