            ...
            <div v-if="rowActive.delete_time == null">
              ...
              >
                ...
                <DropdownMenu slot="list">
                  ...
                  <DropdownItem
                    :disabled="openErp"
                    v-if="[0,1,4,5].includes(orderData.refund_type) && orderData.paid == 1 &&
                        orderData.refund_status !== 2 && parseFloat(orderData.pay_price) > 0"
                    name="5"
                    >立即退款
                  </DropdownItem>
                  <!--  &&
                        (!rowActive.split.length || (rowActive.split.length && orderData._status._type == 1)) -->
                  <DropdownItem
                    :disabled="openErp"
        ...
        <Tabs v-model="activeName">
          <TabPane label="订单信息" name="detail">
            ...
            >
              ...
              <ul class="list">
                <li class="item" v-if="orderDatalist.orderInfo.apply_type == 2">
                  <div>物流公司：</div>
                  <div class="value">
            ...
            <div class="section">
              ...
              <ul class="list">
                ...
                <li class="item">
                  <div>首单优惠：</div>
                  <div class="value">
                    ￥{{ orderDatalist.orderInfo.first_order_price || 0.0 }}
                  </div>
                </li>
                <li
                  class="item"
                ...
                <li class="item" v-if="orderDatalist.orderInfo.type === 4">
                  <div>实付积分：</div>
                  <div class="value">
          ...
          <TabPane label="商品信息" name="product">
            ...
            >
              ...
              <template slot-scope="{ row }" slot="code">
                <!--                 params.row.productInfo.spec_type ? params.row.productInfo.attrInfo.code : params.row.productInfo.code-->
                <span>{{
                  row.productInfo.spec_type
                    ? row.productInfo.attrInfo.code
...
export default {
  ...
  data() {
    return {
      ...
      columns1: [
        ...
        //   title: '商品编码',
        //   render: (h, params) => {
        //     return h('div', params.row.productInfo.spec_type ? params.row.productInfo.attrInfo.code : params.row.productInfo.code);
        //   },
        // },
        {
          title: "商品信息",
  ...
  watch: {
    orderDatalist(value) {
      ...
      if (value.orderInfo.shipping_type == 2 && value.orderInfo.status == 2) {
        this.showWhiteOff = true;
        this.getWriteoffRecord();
      }
    },
    orderData(value) {
  ...
  methods: {
    getWriteoffRecord() {
      writeoffRecordAPi(this.orderId)
        .then((res) => {
          this.offRecordList = res.data;
    ...
    changeMenu(value, row, name, num) {
      if (num) {
        ...
        if (value == 5) {
          let rowData = this.splitList.length ? this.splitList[0] : this.rowActive;
          this.$parent.changeMenu(rowData, value);
        } else {
