  ...
  <div class="form-submit">
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title">
          <router-link
            :to="{ path: '/admin/marketing/store_combination/index' }"
          >
                ...
                <FormItem label="虚拟成团比例(%)：" prop="virtual">
                  <div class="acea-row row-middle">
                    <InputNumber
                      :min="0"
                      disabled
                      placeholder="请输入虚拟成团比例"
                      :precision="0"
                      :max="100"
                      element-id="virtual"
                      v-model="formValidate.virtual"
                      class="perW40"
                  ...
                  <div class="tips">
                    拼团结束前若用户发起的活动未成团，并且现有人数比例大于设置比例，自动补齐人数，拼团成功。100%为不自动补齐
                  </div>
                </FormItem>
              </Col>
              <Col span="24">
...
export default {
  ...
  methods: {
    ...
    peopleNumchange(n) {
      if (n != 0) {
        this.formValidate.virtual = Math.floor(
          ((this.formValidate.people - n) / this.formValidate.people) * 100
        );
      }
    },
    // 拼团规格；
      ...
      combinationInfoApi(this.$route.params.id)
        .then(async (res) => {
          ...
          if (parseInt(this.formValidate.virtual) !== 100) {
            this.formValidate.peopleNum = Math.floor(
              this.formValidate.people -
                (this.formValidate.virtual / 100) * this.formValidate.people
            );
          } else {
