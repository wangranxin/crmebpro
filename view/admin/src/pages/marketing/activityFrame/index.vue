  ...
  <div>
      ...
      <div class="new_card_pd">
        ...
        >
          ...
          <div style="display: inline-block;">
            ...
            <Button type="primary" @click="selChange" class="mr14 mt1"
              >查询</Button
            >
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        ...
        <template slot-scope="{ row }" slot="start_status">
          <Tag color="red" size="medium" v-show="row.start_status === 0"
            >未开始</Tag
          >
          <Tag color="green" size="medium" v-show="row.start_status === 1"
            >进行中</Tag
          >
          <Tag color="default" size="medium" v-show="row.start_status === -1"
            >已结束</Tag
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    onchangeIsShow(row) {
      activityFrameStatus(row.id, row.status)
        .then((res) => {
          this.$Message.success(res.msg);
        })
        .catch((err) => {
    ...
    add() {
      this.$router.push({
        path: "/admin/marketing/activity_frame/create/" + 0,
      });
    },
    // 边框列表
    activityFrameList() {
