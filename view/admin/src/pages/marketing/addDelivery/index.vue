<template>
  <div class="form-submit">
      ...
      >
        <Row :gutter="24" type="flex" v-show="currentTab === '1'">
          ...
          <Col span="24">
            <FormItem label="关联标签：">
              ...
              <div
                class="acea-row row-middle mt10"
                v-if="formValidate.is_label"
              >
                <div
          ...
          <Col span="24">
            <FormItem
              label="优惠内容："
              prop="promotions_cate"
              style="margin-bottom: 0;"
            >
              <RadioGroup
                v-model="formValidate.promotions_cate"
                vertical
                @on-change="changeFull"
              >
                <Radio label="1">
              ...
              <div class="list">
                ...
                >
                  <div
                    ...
                    v-if="formValidate.promotions_cate == 1"
                  >
                    <span>{{ indexw + 1 }}级优惠</span>
                    <span class="del" @click="delGrade(indexw)" v-if="indexw"
                      >删除</span
                    >
                  </div>
                  <FormItem label="优惠门槛：">
                    <span class="mr10"
                      ><span v-if="formValidate.promotions_cate == 2">每</span
                      >满</span
                    >
                    <InputNumber
                      :max="999999"
                      ...
                      class="width23"
                      v-if="formValidate.threshold_type == 1"
                    ></InputNumber>
                    <InputNumber
                      :max="999999"
                      :min="0"
                    ...
                    ></InputNumber>
                    <span class="ml10">{{
                      formValidate.threshold_type == 1 ? "元" : "件"
                    }}</span>
                  </FormItem>
                  <FormItem label="优惠内容：">
                    <Checkbox v-model="item.checkIntegral">送</Checkbox
                    ><InputNumber
                      :max="999999"
                      :min="0"
                      :precision="0"
                      v-model="item.give_integral"
                      class="width23"
                    ></InputNumber
                  ...
                  <div class="ml90">
                    ...
                    <div class="bnt" @click="addCoupon(indexw)">
                      + 添加优惠券
                    </div>
                    ...
                    <Table
                      border
                      :columns="colGoods"
                      :data="item.giveProducts"
                      ref="table"
                      class="table"
                      width="700"
                      v-if="item.giveProducts.length"
                    >
                      <template slot-scope="{ row }" slot="coupon_type">
                        <span v-if="row.coupon_type === 1">满减券</span>
                        <span v-else>折扣券</span>
                    ...
                    <div class="bnt" @click="addGoodsSattr(indexw)">
                      + 添加赠品
                    </div>
                  </div>
                </div>
        ...
        <div v-show="currentTab === '2'">
          <Row :gutter="24" type="flex" class="mb20">
            <Col span="24">
              <RadioGroup
                v-model="formValidate.product_partake_type"
                @on-change="goodTap"
              >
    ...
    >
      <Form>
        <FormItem>
          <Button v-if="currentTab !== '1'" @click="upTab" class="mr10"
            >上一步</Button
          >
          <Button
            type="primary"
...
export default {
            ...
            return h("div", [
              h("InputNumber", {
                ...
                on: {
                  "on-change": (e) => {
                    params.row.limit_num = e;
                    this.promotionsData[params.row.indexGoods].giveProducts[
                      params.index
                    ].limit_num = e;
                  },
                },
  ...
  methods: {
    ...
    getDiscountInfo() {
      discountInfo(this.$route.params.id)
        .then((res) => {
          ...
          promotionsData.forEach((item, index) => {
            ...
            item.checkIntegral =
              parseFloat(item.give_integral) > 0 ? true : false;
    ...
    downTab(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          if (
            parseInt(this.formValidate.is_overlay) &&
            !this.formValidate.overlay.length
          ) {
    ...
    handleSubmit:Debounce(function(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          if (
            this.formValidate.product_partake_type == 4 &&
            !this.formValidate.brand_id.length
          ) {
          ...
          this.promotionsData.forEach((item) => {
            ...
            if (
              item.checkIntegral &&
              (item.give_integral == null || item.give_integral == 0)
            ) {
              this.flag = false;
          ...
          if (
            this.formValidate.product_partake_type == 2 &&
            !product_id.length
          ) {
          ...
          savePromotions(4, this.$route.params.id, this.formValidate)
            .then((res) => {
