					...
					<view class="relative w-full h-full">
						...
						<view class="pl-24 pr-20" :class="!threeCateList.length ? 'mt-24' : ''">
							<scroll-view :scroll-top="0" scroll-y="true" @scrolltolower="lower" @scroll="scroll"
								...
								<!-- 大图模板 -->
								<view v-if="(showType == 3 && level == 2) || (showType == 1 && level == 3)">
								...
								<!-- 小图模板 -->
								<view v-else-if="(showType == 2 && level == 2) || (showType == 2 && level == 3)">
	...
	export default {
		...
		methods: {
			...
			productslist() {
				...
				that.loadTitle = '';
				let is_big = null;
				if((this.showType == 3 && this.level == 2) || (this.showType == 1 && this.level == 3)){
					is_big = 1;
				}
				getProductslist({
					page: that.page,
