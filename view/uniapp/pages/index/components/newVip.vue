<template>
	<view :style="[newVipWrapStyle]" v-if="(couponList.length || productList.length || newcomer_integral) && !dataConfig.isHide">
		<view class="newVip2" :style="[newVip2Background,newVipBorderRadius]" v-if="dataConfig.styleConfig.tabVal">
			<view class="header acea-row row-between row-middle">
				...
				<view class="more" @click="goNewList">去逛逛<text class="iconfont icon-ic_rightarrow"></text></view>
			</view>
			<view class="wrapper">
						...
						<scroll-view scroll-x="true">
							<view class="list acea-row">
								<view v-for="item in couponList" :key="item.id" class="item">
									...
									<view v-if="item.use_min_price" class="info" :style="[couponInfoStyle]">满{{ item.use_min_price }}可用</view>
									<view v-else class="info" :style="[couponInfoStyle]">无门槛券</view>
								</view>
							</view>
						</scroll-view>
					</view>
						...
						<scroll-view scroll-x="true">
							<view class="list acea-row">
								<view class="item" v-for="(item,index) in productList" :key="item.id" @click="goDetail(item)">
									<easy-loadimage :image-src="item.image" width="144rpx" height="144rpx" borderRadius="12rpx"></easy-loadimage>
									<view class="money">¥<text class="number">{{ item.price }}</text></view>
			...
			<view class="bonus acea-row row-middle" v-if="dataConfig.checkboxInfo.type.includes(0)">
				...
				<view class="button" :style="[buttonStyle]" @click="goNewList">去看看</view>
			</view>
		</view>
		<view class="newVip" v-else>
			<view class="header acea-row row-between row-middle">
				<view class="title">新人专享福利</view>
				<view class="more" @click="goNewList">更多优惠<text class="iconfont icon-ic_rightarrow"></text></view>
			</view>
			<view class="bonus" :style="[bonusStyle]" v-if="dataConfig.checkboxInfo.type.includes(0)">
				<view class="inner acea-row row-middle">
					<image :src="`${imgHost}/statics/images/newVip3.png`" class="image"></image>
					新用户注册即可
					<text class="red" :style="[bonusRedStyle]">赠送积分</text>
				</view>
			</view>
			...
			<view class="product" v-if="productList.length && dataConfig.checkboxInfo.type.includes(2)">
				...
				<view class="content">
					<scroll-view scroll-x="true">
						<view class="list acea-row">
							<view class="item" v-for="(item,index) in productList" :key="item.id" @click="goDetail(item)">
								<easy-loadimage :image-src="item.image" width="158rpx" height="158rpx" borderRadius="12rpx"></easy-loadimage>
								<view class="name line1">{{ item.store_name }}</view>
