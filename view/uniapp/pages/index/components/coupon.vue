<template>
	<!-- 优惠券 -->
	<view :style="[couponWrapStyle]" v-show="!dataConfig.isHide && couponList.length">
		<view class="coupon-wrap" :style="[couponWrapBgColor]">
				...
				<scroll-view scroll-x="true">
					<view class="list acea-row">
						<view v-for="item in couponList" :key="item.id" class="item" :style="[couponItemFirstStyle]">
							<view class="text">
								<view class="money" :style="[couponMoneyColor]">
									...
									<template v-else-if="item.coupon_type == 2">
										<text class="number">{{ item.coupon_price / 10 }}</text>折
									</template>
								</view>
								<view class="info">
							...
							<view v-if="item.is_use == true" class="button" :style="[bntBgColor]">已领取</view>
							<view v-else-if="item.is_use == false" class="button" :style="[bntBgColor]" @click="receiveCoupon(item)">去领取</view>
							<view v-else-if="item.is_use == 2" class="button" :style="[bntBgColor]">已过期</view>
						</view>
					</view>
				</scroll-view>
			</view>
			<view v-else-if="dataConfig.styleConfig.tabVal == 1" class="coupon2">
				<scroll-view scroll-x="true">
					<view class="list acea-row">
			...
			<view v-else-if="dataConfig.styleConfig.tabVal == 2" class="coupon3" :style="[coupon3Color]">
				<scroll-view scroll-x="true">
					<view class="list acea-row">
						<view v-for="item in couponList" :key="item.id" class="item">
							<view class="inner acea-row">
								...
								<view v-if="item.is_use == true" class="button acea-row row-middle">已领取</view>
								<view v-else-if="item.is_use == false" class="button acea-row row-middle" @click="receiveCoupon(item)">立即领取</view>
								<view v-else-if="item.is_use == 2" class="button acea-row row-middle">已过期</view>
							</view>
						</view>
					</view>
				</scroll-view>
			...
			<view v-else-if="dataConfig.styleConfig.tabVal == 4" class="coupon5">
				<scroll-view scroll-x="true">
					<view class="list acea-row">
						<view v-for="item in couponList" :key="item.id" class="item acea-row" :style="[bntBgColor]">
							...
							<view v-if="item.is_use == true" class="button acea-row row-middle">已领取</view>
							<view v-else-if="item.is_use == false" class="button acea-row row-middle" @click="receiveCoupon(item)">领取</view>
							<view v-else-if="item.is_use == 2" class="button acea-row row-middle">已过期</view>
						</view>
					</view>
				</scroll-view>
	...
	export default {
		...
		computed: {
			...
			couponItemStyle() {
				...
				// };
				// if (this.dataConfig.toneConfig.tabVal) {
				// 	styleObject['background'] = this.dataConfig.couponBgColor.color[0].item;
