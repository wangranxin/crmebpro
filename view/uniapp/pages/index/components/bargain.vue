<template>
	<view :style="[boxStyle]" v-if="bargList.length && !dataConfig.isHide">
		<view :style="[boxContentStyle]">
			...
			<view class="pt-32 pr-20 pb-32 pl-20 bg--w111-fff" :style="[contentBox]"
				...
				<view class="w-full flex justify-between item"
					v-for="(item,index) in bargList" :key="index"
					@tap="bargDetail(item)">
					<easy-loadimage
			...
			<view class="grid-column-2 grid-gap-22rpx pt-32 pr-20 pb-32 pl-20"
				...
				<view v-for="(item,index) in bargList" :key="index"
					@tap="bargDetail(item)">
					<easy-loadimage
					:image-src="item.image"
					width="100%"
					...
					<view class="flex justify-between items-end mt-10">
						<view class="flex-col">
							...
							v-if="checkboxInfo.includes(2)"></baseMoney>
							<text class="text-line fs-28 text--w111-999 pt-14 Regular"
								v-if="checkboxInfo.includes(3)" :style="[otPriceColor]">¥{{item.price}}</text>
						</view>
			...
			<view class="grid-column-3 grid-gap-18rpx pt-32 pr-20 pb-32 pl-20"
				...
				<view v-for="(item,index) in bargList" :key="index"
					...
					<view class="flex items-baseline">
						<baseMoney :money="item.min_price"
						symbolSize="24"
						integerSize="36"
						decimalSize="36"
						weight
					...
					<view class="text-line fs-24 text--w111-999 Regular lh-32rpx"
						:style="[otPriceColor]" v-if="checkboxInfo.includes(3)">¥{{item.price}}</view>
				</view>
			</view>
			<!-- 滑动 -->
			<scroll-view scroll-x="true" show-scrollbar="false"
				...
				<view class="inline-block ml-20" v-for="(item,index) in bargList" :key="index"
					@tap="bargDetail(item)">
					<easy-loadimage
					:image-src="item.image"
					width="224rpx"
					height="224rpx"
					:borderRadius="imgStyle"></easy-loadimage>
					<view class="w-222 line1 mt-16 fs-26 mb-10" :style="[productStyle]"
						v-if="checkboxInfo.includes(0)">{{item.title}}</view>
					<text class="badge fs-20 text--w111-fff mb-14"
						v-if="checkboxInfo.includes(0)" :style="[braginSuccess]">{{item.people}}人已砍成功</text>
					<view>
						<baseMoney :money="item.min_price"
						symbolSize="24"
						integerSize="36"
		...
		computed:{
			...
			braginSuccess(){
				...
				if (this.dataConfig.toneConfig.tabVal) {
					...
					styleObject['color'] = this.dataConfig.joinNumColor2.color[0].item;
				}
				return styleObject;
			},
			/*商品数量*/
