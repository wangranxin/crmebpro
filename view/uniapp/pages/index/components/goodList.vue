<template>
	<view :style="[boxStyle]" v-if="tempArr.length && !dataConfig.isHide">
		<view>
			<!-- 单列 -->
			...
			<!-- 横向滑动 -->
			<view class="pt-32 pb-32 pl-24 rd-16rpx" :style="[moduleColor]" v-if="goodStyleConfig == 5">
	...
	export default {
		...
		computed:{
			...
			wfItemStyle(){
				...
				return {
					width: (windowWidth - 10 - (this.dataConfig.prConfig.val * 2))/2 + 'px'
				}
			},
			/*商品图片圆角样式*/
			imgStyle(){
