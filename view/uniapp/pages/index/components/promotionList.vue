<template>
	...
	<view v-if="!dataConfig.isHide">
		<view class="index-product-wrapper" :style="[bottomBgColor]">
			<view :style="{height:navBdH+'px'}" class="nav-bd-box">
				...
				'nav-bd5': dataConfig.styleConfig.tabVal == 4,
					...
					<scroll-view class="scroll-view" scroll-x="true">
						<view class="item" 
						v-for="(item,index) in explosiveMoney" :key="index" 
...
import homeCombVue from './homeComb.vue';
	export default {
		name: 'promotionList',
		props: {
			dataConfig: {
		...
		computed: {
			numConfig(){
				return this.dataConfig.tabConfig.list[this.ProductNavindex].numConfig.val
			},
			decorateColor() {
			...
			fixedTop(){
				if(this.isHeaderSerch || this.showHomeComb){
					return (this.statusBarHeight + 43)+'px'
				}else{
			...
			fixedStyle(){
				return {
					...
					top:this.fixedTop,
					background:this.sticky?'#FFFFFF':'none',
				}
			}
		},
		watch: {
			activeValue: {
				handler(value) {
					...
					let goodDataConfig = {
						...
						prConfig: {
							val: this.dataConfig.prConfig.val,
						},
						bottomConfig: {
							val: 0,
						},
		...
		mounted() {
			...
			let positionTop = this.statusBarHeight + 43;
			...
			if (!this.dataConfig.slideConfig.tabVal) {
				uni.$on('onPageScroll', () => {
					views.boundingClientRect((data) => {
						...
						this.sticky = data.top <= positionTop;
					}).exec();
				});
			}
		},
