<template>
	<view class="page_count" v-if="!dataConfig.isHide">
		<view class="bg-img">
			<image :src="bgColor"></image>
		</view>
		...
		<view class="my-main">
			<view class="mp-header" id="home" :style="[mpHeaderStyle]">
				...
				<view class="serch-box">
					<view class="serch-wrapper flex">
						...
						<navigator v-if="hotWords.length" :url="'/pages/goods/goods_search/index?searchVal='+searchVal" :class="logoConfig ? 'input' : 'uninput'" hover-class="none"
							class="skeleton-rect" :style="[inputStyle]">
				...
				<view class="longTab" :style="{ width: `${mainWidth}px` }">
					<scroll-view scroll-x="true" scroll-with-animation :scroll-left="tabLeft" show-scrollbar="true">
						<view class="longItem" v-for="(item,index) in tabListConfig" :key="index" 
							...
							:style=[longItemStyle]
							@click="changeTab(item,index)">{{item.text.val}}</view>
					</scroll-view>
				</view>
			...
			<view v-if="isCategory" class="category_count">
				...
				<view class="cate_count grid-column-4 grid-gap-16rpx mt-32">
					<view class="category_item" :class="index===tabClick?'clicks':''" @click="changeTab(item,index)" v-for="(item,index) in tabListConfig" :key="index" :id="'ids'+index">
						{{item.text.val}}
					</view>
				</view>
			</view>
		</view>
		<view class="swiperBg pb-20">
			<!-- :style="{ paddingBottom: isMenu ? '30rpx' : '30rpx'}" -->
			<view class="swiper page_swiper" v-if="imgUrls.length">
				...
				<view class="dots acea-row" :class="{
					...
					<view v-else class="acea-row" :class="{
						...
						<view v-for="(item, index) in imgUrls" :key="index" class="dot" :class="{ active: index == swiperCur }" :style="[dotBgColor,index==swiperCur?dotColor:'']"></view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			bgGradientStyle(){
				return {
					'background-image': `linear-gradient(to bottom, rgba(245,245,245,0) 0%, rgba(245,245,245,0) 50%, ${this.dataConfig.gradientColor ? this.dataConfig.gradientColor.color[0].item : '#f5f5f5'} 100%)`
				}
			},
			mpHeaderStyle() {
				let style = {};
			...
			longItemStyle(){
				return {
					marginRight: `${this.dataConfig.contentConfig.val}px`,
				};
			},
			logoUpImg() {
				let img = '';
		...
		methods: {
			...
			computedHeight(){
				...
				uni.getImageInfo({
					...
					success: (res) => {
						let height = res.height * ((windowWidth - 20) / res.width)
						that.$set(that, 'imageH', height.toFixed(2));	
					}
				})
			},
			//替换安全域名
