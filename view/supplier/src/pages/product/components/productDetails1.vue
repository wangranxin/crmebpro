    ...
    >
      <div class="drawer-main">
        ...
        >
            ...
            <Col span="24">
              <FormItem label="商品轮播图：">
                <div class="acea-row">
                  ...
                  <div
                    v-if="formValidate.slider_image.length < 10 && !pid"
          ...
          <Row type="flex" v-show="currentTab === '2'">
            <Col span="24">
            ...
            <Col span="24" v-if="formValidate.spec_type === 1" class="noForm">
              <Col span="24">
                <FormItem label="选择规格：">
                  <div class="acea-row row-middle">
                    <Select
                      v-model="formValidate.selectRule"
                      style="width: 23%"
                    >
                      <Option
...
<!--                      </template>-->
                      ...
                      >
                        <Button
                          v-if="!row.virtual_list && !row.stock"
                        ...
                        <span
                          ...
                          class="seeCatMy"
                          @click="
                            seeVirtual(oneFormBatch[0], 'oneFormBatch', 0)
                          "
                          >已设置</span
                        >
                      </template>
                ...
                <Col span="24">
                  <FormItem label="商品属性：" class="labeltop">
                    ...
                    >
                      <template
                        slot-scope="{ row, index }"
                        v-for="(item, i) in attrData.length"
                        :slot="'value' + (i + 1)"
                      >
                        <div
                          :class="
                            manyFormValidate[index].select ? 'selectOn' : ''
                          "
                        >
...
<!--                      </template>-->
                      ...
                      <template slot-scope="{ row, index }" slot="bar_code">
                        <Input
                          v-model="manyFormValidate[index].bar_code"
                        ></Input>
                      </template>
                      <template slot-scope="{ row, index }" slot="code">
              ...
              <Col span="24" v-if="formValidate.product_type == 1">
                <FormItem label="卡密设置：">
                  ...
                  <span
                    ...
                    class="seeCatMy"
                    @click="
                      seeVirtual(oneFormValidate[0], 'oneFormValidate', 0)
                    "
                    >已设置</span
                  >
                </FormItem>
...
<!--                </div>-->
            ...
            <Col span="22" offset="1">
              ...
              >
                ...
                <template slot-scope="{ row }" slot="merchant_reply_content">
                  <div class="mb5 content_font" v-if="row.replyComment">
                    {{ row.replyComment.content }}
                  </div>
                </template>
                <template slot-scope="{ row, index }" slot="action">
        ...
        <Form>
          <FormItem>
            ...
            <Button
              ...
              v-if="!pid && (productId || currentTab === '5')"
              >保存</Button
            >
          </FormItem>
        </Form>
    ...
    <Modal v-model="replyModal" scrollable title="回复内容" closable>
      <Form ref="replyForm" :model="replyForm" :rules="ruleInline" :label-position="labelPosition" @submit.native.prevent>
...
import menusFrom from './menusFrom';
...
export default {
  ...
  mounted() {
    ...
    if ((this.productId !== '0' && this.productId) || this.$route.query.copy) {
      this.getInfo();
  ...
  methods: {
    ...mapMutations('layout', ['setCopyrightShow']),
    changeForm(e){
    ...
    cancel() {
      this.$router.push({ path: `/supplier/product/index` });
    },
    ...
    getInfo() {
      let that = this;
      that.spinShow = true;
      productInfoApi(that.productId || this.$route.query.copy)
        .then((res) => {
    ...
    getReplyList() {
      ...
      productReplyApi(this.replyValidate)
        .then(async (res) => {
          let data = res.data;
