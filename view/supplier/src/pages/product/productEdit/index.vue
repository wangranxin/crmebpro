          ...
          <FormItem label="商品规格：">
            <div class="flex-y-center">
              ...
              <Dropdown v-if="formData.spec_type == 1" @on-click="confirm">
                <span class="pl-14 text-blue pointer" style="position: relative;top: 2px;">
                  选择规格模板
                      ...
                      <div class="rulesBox ml30">
                        ...
                        >
                          ...
                          <el-popover
                            ...
                            :style="{'min-height': item.add_pic ? '121px' : ''}"
                          >
                            <Input
                              :ref="'inputRef_' + index"
                              placeholder="请输入规格值"
...
export default {
  ...
  methods: {
        ...
        for (let i = 0; i < combination.length; i++) {
          ...
          for (let k = 0; k < this.manyFormValidate.length; k++) {
            ...
            if (k > 0 && attrDetail && arraysEqual(attrDetail, combination)) {
              Object.assign(row, {
                settle_price: manyItem.settle_price,
    ...
    infoData(data){
      ...
      if (data.spec_type == 1) {
        data.attrs.map((item) => {
          this.$set(item, "settle_price", Number(item.settle_price));
    ...
    summarizeData() {
      ...
      let formData = { ...baseSetData, ...otherSetData };
      if (this.$route.query.copy) {
        this.$set(formData, "id", 0);
      }
      this.$set(formData, "type", this.type);
      this.$set(formData, "product_type", 0);
